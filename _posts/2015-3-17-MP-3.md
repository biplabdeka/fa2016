---
layout: default
title: "MP 3.0: Creating and consuming a RESTful API"
...
| filter   | specify the set of fields to include or exclude in each document  (1 - include; 0 - exclude) |
...
| `http://www.uiucwp.com:4000/api/users?filter={"_id": 0}`                                  | Returns a list of users without the _id field           |
...
+ All responses from your API should be a JSON object with two fields. The first field should be named `message` and should contain a human readable String that indicates success or failure (with reason of failure if possible) of the request. The second field should be named `data` and should contain the actual JSON response object. For example, here is a valid response:
...
+ Your API should respond with appropriate [HTTP status codes](http://www.restapitutorial.com/httpstatuscodes.html). You should at least handle the following codes: 200, 201, 404, 500.
...
+ If a user is deleted, all pending tasks that the user had should be updated to have `assignedUserName` changed to "unassigned" 
...
<li> You app should interact with the API only through custom service(s) that you write. You would have to inject these services into your controller. </li>
</ol>
...
```
+ Look at [this stack overflow question](http://stackoverflow.com/questions/13937318/convert-angular-http-get-function-to-a-service) to understand how to start a service
+ [Helpful beginner tutorial](http://adrianmejia.com/blog/2014/09/28/angularjs-tutorial-for-beginners-with-nodejs-expressjs-and-mongodb/)
