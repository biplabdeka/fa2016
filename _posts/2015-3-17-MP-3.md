---
layout: default
title: "MP 3.2: Creating and consuming a RESTful API"
...
<li> Your app should have the following views:
	<ol>
	    ...
	    Task List: 
	    <ul>
	    	<li>List task names (use **pagination** to display only 20 at a time) with the name of the assigned user next to it. The API should be called only to return the needed information about each task (name, _id, assignedUserName). Clicking on a task should take us to the Task Details view.</li>
