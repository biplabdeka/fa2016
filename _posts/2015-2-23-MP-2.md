...
+ You should not use absolute paths to load any of your asset files (say css, images or js) from your server. For example, there should not be something like:
```html
<img src="/photos/skyline.jpg" />
```
They should all be relative paths like:
```html
<img src="./photos/skyline.jpg" />
```
This is needed because when we host your code for grading the folder structure on the server, the folder structure would be different from what it was on your server and things would not work. 
...
+ Uglification/minification of Angular code can result in undesirable changes (due to change in function parameter names). One thing you can do to avoid this is to always decare your controllers as: 
...
```
This prevents the function parameter names from being changes.
