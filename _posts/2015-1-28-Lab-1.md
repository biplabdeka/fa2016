...
source ~/.profile
```
 8 - Go to your github account (or create one) and create a new repository named `mp1`. ***Set it to private*** (you'll have to activate the [Github student developer pack](https://education.github.com/pack) to be able to create private private repos. The other option is to use [bitbucket](https://bitbucket.org/)). Create a duplicate of the [MP-1 starter code](https://github.com/uiuc-web-programming/mp1_starter) from the class github repo by following the commands below 
